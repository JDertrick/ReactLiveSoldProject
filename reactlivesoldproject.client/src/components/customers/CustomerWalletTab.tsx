...
export const CustomerWalletTab = ({ customer }: CustomerWalletTabProps) => {
  ...
  const { data: transactions, isLoading } = useGetWalletTransactions(customer.id);
  ...
  const totalCredits = transactions?.reduce((sum, t) =>
    t.type.toLowerCase() === "credit" ? sum + t.amount : sum, 0
  ) ?? 0;

  const totalDebits = transactions?.reduce((sum, t) =>
    t.type.toLowerCase() === "debit" ? sum + t.amount : sum, 0
  ) ?? 0;
      ...
      <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-2xl p-6 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-start justify-between mb-6">
            <div>
              <p className="text-sm opacity-80 font-medium uppercase tracking-wide">Wallet Balance</p>
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-white/20">
            ...
            <div>
              ...
              <p className="text-xl font-bold mt-1">${totalDebits.toFixed(2)}</p>
            </div>
            <div>
